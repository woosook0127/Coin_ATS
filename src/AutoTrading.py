...
#----------------------------------------------------------------------
    ...
    def activate(self):
        ...
        while True:
            try:
                ...
                self.my_btc = self.upbit.get_balance(self.coin_type)
                apr_price = self.my_btc * pyupbit.get_current_price(self.coin_type) # appraised price
                ...
                if start_time < now < end_time - datetime.timedelta(seconds=120): # don't trading during 2min
                    ...
                    if profit_rate >= 1.25: # 익절: 2.5%
                        self.upbit.sell_market_order(self.coin_type, self.my_btc*0.9995)# 전량 매도 
    ...
    def deactivate(self):
        ...
        try:
            self.upbit.sell_market_order(self.coin_type, self.my_btc*0.9995)
        except Exception as e:
            print(f"Exception: {e}")
            time.sleep(0.1)
